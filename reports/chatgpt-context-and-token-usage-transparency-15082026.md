---
name: chatgpt-context-token-transparency-report
...
requires: []
provides:
- chatgpt-context-token-transparency-report
applies_to:
- aifups
...
ChatGPT asks users to conduct long-running conversations, research projects, and artifact workflows inside an interface that does not give them a clear resource meter for one of the system's most consequential constraints: model context. In this conversation, the assistant could not report the exact token cost of a turn, the accumulated token load of the chat, how much context remained, or precisely what had happened when the product indicated that it was “making room for more conversation.” That makes it difficult for a user to budget a long task, decide whether to start a new chat, preserve important research state, or assess whether a deteriorating interaction is caused by context pressure rather than the work itself.
...
## 3. What “making room for more conversation” likely means

The exact implementation behind that particular ChatGPT UI phrase is not sufficiently documented in the public sources reviewed for this report, so it should not be presented as a verified description of the consumer product. However, OpenAI has publicly described the underlying class of problem and a concrete solution in Codex: when context exceeds a threshold, the system compacts the conversation into a smaller representation and continues from that representation. OpenAI describes a Responses API compaction mechanism that can replace previous input with a smaller set of items, including an opaque compaction representation intended to preserve useful understanding.

OpenAI's GPT-5.6 engineering description likewise warns that instructions, conversation history, tools, skills, plugins, and results can create context bloat, and describes harness techniques for limiting that overhead. Taken together, these sources make compaction or related context-management behavior a strong technical interpretation of “making room,” but they do not establish exactly which ChatGPT messages are summarized, dropped, retrieved later, or preserved verbatim when that specific notice appears.

## 4. Published capacity is not live observability
...
## 5. What is visible to the user today
...
What is conspicuously absent from the ordinary chat interface is a live context ledger: no clear per-turn token estimate, conversation occupancy, remaining context, projected compaction threshold, list of material currently resident in context, or audit of what changed during a “making room” event.

## 6. What appears deliberately non-visible versus merely undocumented
...
## 7. Problems created for budgeting and planning
...
## 8. Ongoing research is unusually vulnerable
...
## 9. Recommended user-facing transparency
...
## 10. Dark-UX concerns and open questions for OpenAI
...
- When ChatGPT says it is “making room for more conversation,” exactly what operation is occurring in that product surface?
...
Not verified from the public sources reviewed: that the exact consumer ChatGPT phrase “making room for more conversation” maps one-to-one to the same compaction implementation documented for Codex; the precise threshold used in this chat; the exact token occupancy of this chat or any turn; the precise material compressed or omitted during any prior event; or OpenAI's internal rationale for withholding a live context meter.
